...
def get_forecast_lr(target, data):
    ...
    for day in range(3):
        ...
        X_pred = np.array([[
            ...
            last_values[-1], last_values[-2] if len(last_values) >= 2 else last_values[-1],
...
def get_forecast_pm25(data):
    ...
    for day in range(3):
        ...
        pred = PM25_MODEL.predict(X_pred)[0]
        results.append(round(pred, 2))
        last_values.append(pred)
...
def get_forcast_verdicts(pm25_forecast, pm10_forecast, so2_forecast, co_forecast, no2_forecast):
    ...
    day1_verdict = "плохое" if day1_aqi <= 4 else "хорошее" if day1_aqi >= 8 else "нормальное"
    day2_verdict = "плохое" if day2_aqi <= 4 else "хорошее" if day2_aqi >= 8 else "нормальное"
    day3_verdict = "плохое" if day3_aqi <= 4 else "хорошее" if day3_aqi >= 8 else "нормальное"
        ...
        'predictions': [
            {
                ...
                'quality_verdict': forecast_verdicts["day1_verdict"],
                'pollutants': {
            ...
            },
            {
                ...
                'quality_verdict': forecast_verdicts["day2_verdict"],
                'pollutants': {
            ...
            },
            {
                ...
                'quality_verdict': forecast_verdicts["day3_verdict"],
                'pollutants': {
...
def history(request):
    ...
    with open(file_path3, mode='r', newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            try:
                date_obj = datetime.strptime(date_str.zfill(5), '%d.%m').replace(year=datetime.now().year)
            except ValueError:
                continue
            ...
            verdict_sum = get_day_verdict({'pm25': pm25, 'pm10': pm10, 'so2': so2, 'co': co, 'no2': no2})
            row_data['day_verdict'] = 'Плохо' if verdict_sum <= 4 else 'Хорошо' if verdict_sum >= 8 else 'Нормально'
    ...
    with open(file_path1, "r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            verdict_sum = get_day_verdict({'pm25': float(row_data['pm25']), 'pm10': float(row_data['pm10']), 'so2': so2_val, 'co': co_val, 'no2': no2_val})
            row_data['day_verdict'] = 'Плохо' if verdict_sum <= 4 else 'Хорошо' if verdict_sum >= 8 else 'Нормально'
